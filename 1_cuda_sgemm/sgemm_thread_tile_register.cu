                                       ...
                                       const int K)
{
    ...
    for (int step = 0; step < K; step += K_PER_BLOCK)
    {
        ...
        {
            // 1. 加载A的子块到共享内存
            FLOAT4(s_a[ty * NUM_PER_THREAD + vec_y][tx * NUM_PER_THREAD]) = FLOAT4(a_ptr[(row + vec_y) * K + (tx * NUM_PER_THREAD + step)]);
            // 2. 加载B的子块到共享内存
...
#pragma unroll
        ...
        {
            a_reg[0] = s_a[ty * REG_NUM][k];
            a_reg[1] = s_a[ty * REG_NUM + 1][k];
            a_reg[2] = s_a[ty * REG_NUM + 2][k];
            a_reg[3] = s_a[ty * REG_NUM + 3][k];

 ...
 */
__global__ void cuda_sgemm(float *a_ptr,
                           float *b_ptr,
                           float *c_ptr,
                           const int M,
                           const int N,
                           const int K)
{
    ...
    {
        // 1. 加载A的子块到共享内存
        s_a[ty][tx] = a_ptr[row * K + (tx + step)];
        // 2. 加载B的子块到共享内存
        s_b[ty][tx] = b_ptr[(ty + step) * N + col];
...
int main()
{
    ...
    cuda_sgemm<<<grid, block>>>(device_a, device_b, device_c, m, n, k);
